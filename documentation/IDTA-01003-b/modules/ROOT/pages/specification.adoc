...
class DataSpecificationUnitOfMeasure<DataSpecificationContent> <<Template>> {
  ...
  +lassificationSystem: string[0..1]
}
@enduml
...
e|id e|IRDI e|Code |_prefix_ footnote:prefix[see Table <<Prefixes>>] + _unitId_  |_prefix_ footnote:prefix[] + _unitId_ e|PID footnote:[Example for BIPM PID: https://si-digital-framework.org/SI/units/ampere]
...
e|preferredNameQuantity  e|Quantity e| e|Quantity e|QuantityType e|Quantity
e|quantityID  e| e| |--- |--- e|PID (of Quantity)
...
|===
|Dictionary |_prefix_ |Example
|UNECE e|"uncefact:UNECERec20Code/" e|"uncefact:UNECERec20Code/MMT"

|OPC UA |"http://www.opcfoundation.org/UA/units/" e|"http://www.opcfoundation.org/UA/units/ 
|BIPM  e|Short name e|Short name 
|===


====
Note: OPC UA uses the attribute _namespaceUri_ to denote the classification system. It is recommended to reference the original dictionary to avoid "double-mapping", if possible. 
...
- The *Symbol* field shall be used as invariant locale for _displayName_.
- The *Name* field shall be used as invariant locale for _description_. 
====